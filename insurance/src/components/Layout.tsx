...
const NAV = [
  ...
  { to: '/monitor', label: 'Trigger Monitor', icon: '⚡' },
  { to: '/events', label: 'Events Log', icon: '☰' },
  { to: '/simulate', label: 'Payout Simulation', icon: '€' },
]
    ...
    <div style={shell}>
      <aside style={sidebar} aria-label="Primary">
        <div style={brand}>
          ...
          <span style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>Insurance</span>
        </div>
        <nav style={nav}>
          {NAV.map(item => (
            <NavLink
        ...
        <div style={footer}>
          <div style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>Parametric HAB Cover</div>
        </div>
      </aside>
      <main style={main}>
        <Outlet />
...
const navActive: CSSProperties = {
  ...
  borderLeft: '3px solid #fbbf24', paddingLeft: 9,
}
